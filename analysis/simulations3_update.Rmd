...
kset <- (length(fit2$pve) - rank(fit2$pve) < 11) & (fit2$pve > 0)
kall <- 1:fit2$n.factors
fit2 <- flash.remove.factors(fit2, kset=kall[!kset])
fit2 <- fit.ebcovmf(dat=XXt, fl=fit2, prior=as.prior(ebnm::ebnm_point_exponential, sign = 1))$fl
...
fit3 <- readRDS("output/simulations3_update_XXt_fit3_sigma_v1.Rds")
...
pheatmap(fit3.L, show_rownames = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, main="Estimated L with point-exponential prior \n (iteratively greedy + backfit, sigma2=1)")
...
```{r warning=F}
### fit flash with point exponential prior to XXt 
fit3 <- readRDS("output/simulations3_update_XXt_fit3_sigma_v2.Rds")
